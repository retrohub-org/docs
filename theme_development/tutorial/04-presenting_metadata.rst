...
.. code:: gdscript
	...
	func _on_game_received(game_data: RetroHubGameData):
		if game_data.system_name == system_data.name:
...
You can run the project now, and try launching a game. You'll see that no game is actually launching (this is random data anyways), but if you look at the console output, you'll see messages indicating that a game is launched. This means everything is working; if this theme was running under RetroHub right now, it would launch the game!
